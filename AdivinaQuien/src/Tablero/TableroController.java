...
public class TableroController extends MenuController implements Initializable, ClienteListener, MensajeListener {
    ...
    @Override
    public void onIniciarPartida(String oponenteNick, List<Personaje> personajesRecibidos) {
        Platform.runLater(() -> {
            this.nickNameOp = oponenteNick;
            this.labelJugador2.setText(oponenteNick);
            this.personajesJuego = personajesRecibidos;

            gridTable.getChildren().clear();
            imagenes.clear();
            chat.getChildren().clear();
            segundosTranscurridos = 0L;
            volteados = 0;
            esMiTurno = false;
    ...
    private void cargarListaPreguntas() {
        ...
        listViewPreguntas.setCellFactory(param -> new ListCell<String>() {
            ...
            { // Se crea el Layout para el panel de la pregunta y el botón
                ...
                labelPregunta.getStyleClass().add("labelPreguntaLista"); // Se le asigna una clase de estilos al label de la pregunta
    ...
    private void reasignarMetodos(){
        for(int i=0; i<24; i++) { // Ciclo para recorrer cada celda del tablero
    ...
    private void voltear(MouseEvent e, int indice) throws IOException {
        ...
        StackPane stack = (StackPane)gridTable.getChildren().get(indice);
            ...
            if (hijo instanceof ImageView) { // Si el hijo es un ImageView
                if(personajesJuego.get(indice).isTachado()){ // Si el personaje actual ya está tachado
                    ...
                    if(this.volteados >= 24){ // Si ya se voltearon todos los personajes
                        mostrarPantallaFinal(false); // false = derrota
                    }
                }
                continue;
            }
    ...
    private void mostrarPantallaFinal(boolean ganeLaPartida) {
        Platform.runLater(() -> {
            try {
                FXMLLoader loader = new FXMLLoader(
                        getClass().getResource("/TerminarPartida/TerminarPartida.fxml")
                );

                Parent root = loader.load();
                TerminarPartidaController finCtrl = loader.getController();
                finCtrl.redireccionPantalla(ganeLaPartida);

                finCtrl.setCliente(this.cliente);
                Scene nuevaScene = new Scene(root);

                nuevaScene.getStylesheets().add(
                        getClass().getResource("/TerminarPartida/TerminarPartidaStyles.css")
                                .toExternalForm()
                );
                ...
                System.err.println("¡ERROR CRÍTICO AL REDIRECCIONAR!");
                ex.printStackTrace();
            }
        });
    }
    ...
    private void adivinar(MouseEvent e, int indice) throws IOException {
        ...
        String mensajeAEnviar = idAdivinar + ":" + this.segundosTranscurridos;
        cliente.enviarMensaje("ADIVINAR", mensajeAEnviar);
    }
    ...
    public void onManejarMensajeServidor(String mensaje) {
        Platform.runLater(() -> {
            if (mensaje.startsWith("TU_TURNO:")) {
                ...
                esMiTurno = nickTurno.equals(miNickname);
                preguntaEnviada = false;
                textFieldMensaje.setDisable(!esMiTurno);
                buttonEnviar.setDisable(!esMiTurno);
                agregarMensajeAlChat("Sistema", esMiTurno ? "Es tu turno, realiza tu pregunta" : "Es el turno del jugador " + nickTurno);
            } else if (mensaje.startsWith("PREGUNTA:")) {
                String[] partes = mensaje.split(":", 3);
                if (partes.length >= 3 && !partes[1].trim().equals(miNickname)) {
                    agregarMensajeAlChat(partes[1].trim(), partes[2].trim());
                    mostrarPregunta(partes[2].trim());
                }
                ...
                mostrarPantallaFinal(mensaje.contains("GANASTE"));
                ...
                reloj();
            } else if (mensaje.startsWith("OPONENTE_DESCONECTADO:")) {
                String nickDesconectado = mensaje.substring("OPONENTE_DESCONECTADO:".length()).trim();
                agregarMensajeAlChat("Sistema", "Tu oponente (" + nickDesconectado + ") se ha desconectado.");
                textFieldMensaje.setDisable(true);
                buttonEnviar.setDisable(true);
                if (cliente != null) cliente.desconexion();
            } else {
    ...
    public void terminarTurno(ActionEvent e) {
        ...
        String respuesta = (fuente.getId().equals("btnRespSi")) ? "SI" : "NO";
        cliente.enviarMensaje("RESPUESTA", respuesta);
        agregarMensajeAlChat(miNickname, respuesta);
        shadowPanePreguntas.setVisible(false);
        gridPaneRespuestas.setVisible(false);
    }
